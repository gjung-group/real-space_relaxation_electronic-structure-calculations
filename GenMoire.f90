program GenMoire
   ...
   double precision :: phideg = 0.0d0 ! Rotation angle in degrees
   double precision, parameter :: degtol=0.01d0, lattol = 0.001d0 ! Tolerance
   double precision, parameter :: h = 3.27d0, dz = 100.0d0
   double precision, parameter :: aG = 2.4602d0, aBN0 = 2.505d0 ! Lattice parameters
   character(*), parameter :: select = 'delta' ! choose angle, delta or both
   ...
   character(*), parameter :: grSp(2) = (/'C','C'/), bnSp(2) =(/'B','N'/)
...
subroutine MoireFind(n1,n2,nM1,nM2,phiin,delta,degtol,lattol,select,mout)
   ...
   nn = a**2 + a*b + b**2
   nn2 = ap**2 + ap*bp + bp**2
   write(*,'(a,4i6,2f16.9,i6,2i6)') 'For angle:  ',a,b,ap,bp,d,alpha*180.0d0/pi,N,nn, nn2
   a = m0p(1)
   b = m0p(2)
   ap = m0p(3)
   bp = m0p(4)
   ...
   nn = a**2 + a*b + b**2
   nn2 = ap**2 + ap*bp + bp**2
   write(*,'(a,4i6,2f16.9,i6,2i6)') 'For delta:  ',a,b,ap,bp,d,alpha*180.0d0/pi,N, nn, nn2
   a = m0(1)
   b = m0(2)
   ap = m0(3)
   bp = m0(4)
   ...
   nn = a**2 + a*b + b**2
   nn2 = ap**2 + ap*bp + bp**2
   write(*,'(a,4i6,2f16.9,i6,2i6)') 'For both:   ',a,b,ap,bp,d,alpha*180.0d0/pi,N,nn,nn2
...
out2:do i=1,nb2
   ...
   write(u2,'(a,3f16.9)') 'c:', 0.0d0, 0.0d0, 100.0d0
   write(u2,*)
   ...
   write(u5,*)
   write(u5,'(i6,a)') sz1+sz2, ' atoms'
   write(u5,'(i1,a)') 3, ' atom types'
   write(u5,*)
   ...
   write(u5,'(a)') ' Masses'
   write(u5,*)
   write(u5,'(i1, 3f16.9)') 1, 10.80
   write(u5,'(i1, 3f16.9)') 2, 14.00
   write(u5,'(i1, 3f16.9)') 3, 12.01
   write(u5,*)
   write(u5,'(a)') ' Atoms'
   write(u5,*)
   ...
   do i=1,sz1
      ...
      write(u4,'(a,i6,a,a)') 'basis ', j, ' 2', ' &'
      write(u5,'(i6,a,i6,a,3f16.9,3i3)') j, ' 1 ', 3 , ' 0.0 ', Xs(:,i), rz, 0, 0, 0 
      write(u6,'(i6,i8)') j, num1(mod(i+1,nb1)+1) 
   end do
   rz = (z/2.0d0 + h/2.0d0)/z + (0*h/z)
   do i=1,sz2
      j = j+1
      write(u,'(3f16.9,i8)') Xo(:,i), rz, num2(mod(i+1,nb2)+1)
      if (num2(mod(i+1,nb2)+1)-2.eq.1) then
         write(u2,'(A8,3f16.9)') 'N', Xo(:,i), rz
      else if (num2(mod(i+1,nb2)+1)-2.eq.2) then
         write(u2,'(A8,3f16.9)') 'B', Xo(:,i), rz
      end if
      write(u3,'(a,3f16.9,a)') 'basis ', Xo(:,i), rz, ' &'
      write(u4,'(a,i6,a,a)') 'basis ', j, ' 1', ' &'
      !write(u5,'(i6,a,3f16.9,3i3)') j, ' 1 ', Xs(:,i), rz, 0, 0, 0 
      if (num2(mod(i+1,nb2)+1)-2.eq.1) then
          write(u5,'(i6,a,i6,a,3f16.9,3i3)') j, ' 2 ', num2(mod(i+1,nb2)+1)-2, ' 0.82275 ', Xs(:,i), rz, 0, 0, 0 
      else
          write(u5,'(i6,a,i6,a,3f16.9,3i3)') j, ' 2 ', num2(mod(i+1,nb2)+1)-2, ' -0.82275 ', Xs(:,i), rz, 0, 0, 0 
      end if
      write(u6,'(i6,i8)') j, num2(mod(i+1,nb2)+1) 
   end do
